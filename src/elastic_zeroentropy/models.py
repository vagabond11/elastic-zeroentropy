...
class Document(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(None, description="When the document was created/indexed")
...
class SearchResult(BaseModel):
    ...
    elasticsearch_score: Optional[float] = Field(None, description="Original Elasticsearch score")
    rerank_score: Optional[float] = Field(None, description="ZeroEntropy reranking score")
...
class ElasticsearchQuery(BaseModel):
    ...
    size: int = Field(default=10, description="Number of results to retrieve", ge=1, le=1000)
    from_: int = Field(default=0, description="Offset for pagination", alias="from", ge=0)
    source: Optional[List[str]] = Field(None, description="Fields to include in response")
...
class RerankRequest(BaseModel):
    ...
    documents: List[str] = Field(..., description="List of document texts to rerank", min_items=1)
...
class RerankerConfig(BaseModel):
    ...
    top_k_initial: int = Field(default=100, description="Number of documents to retrieve from Elasticsearch", ge=1, le=10000)
    top_k_rerank: int = Field(default=20, description="Number of documents to send for reranking", ge=1, le=1000)
    top_k_final: int = Field(default=10, description="Number of final results to return", ge=1, le=100)
    model: str = Field(default="zerank-1", description="ZeroEntropy model to use")
    combine_scores: bool = Field(default=True, description="Whether to combine ES and rerank scores")
    ...
    def top_k_rerank_must_not_exceed_initial(cls, v: int, values: Dict[str, Any]) -> int:
        if 'top_k_initial' in values and v > values['top_k_initial']:
            raise ValueError("top_k_rerank cannot exceed top_k_initial")
        return v
    
    @validator('top_k_final')
    def top_k_final_must_not_exceed_rerank(cls, v: int, values: Dict[str, Any]) -> int:
        if 'top_k_rerank' in values and v > values['top_k_rerank']:
...
class HealthCheckResponse(BaseModel):
    ...
    elasticsearch: Dict[str, Any] = Field(..., description="Elasticsearch connection status")
...
class SearchRequest(BaseModel):
    ...
    query: str = Field(..., description="Search query text")
    elasticsearch_query: Optional[ElasticsearchQuery] = Field(None, description="Custom Elasticsearch query")
    reranker_config: Optional[RerankerConfig] = Field(None, description="Reranking configuration")
