...
from .exceptions import ElasticsearchError, ConfigurationError, TimeoutError as LibTimeoutError
...
class ElasticsearchClient:
    ...
    async def _ensure_client(self) -> None:
        ...
        if self._client is None:
            ...
            try:
                ...
                logger.info(f"Connected to Elasticsearch at {self.config.elasticsearch_url}")
                
            except Exception as e:
                raise ElasticsearchError(
                    f"Failed to connect to Elasticsearch: {e}",
                    elasticsearch_error=e
                ) from e
    ...
    def _handle_elasticsearch_error(self, error: Exception, operation: str, **kwargs: Any) -> None:
        """
    ...
    def _extract_documents_from_response(self, response: Dict[str, Any]) -> List[Document]:
        """
                ...
                if field in source:
                    try:
                        if isinstance(source[field], str):
                            timestamp = datetime.fromisoformat(source[field].replace('Z', '+00:00'))
    ...
    async def search(
        ...
        try:
            ...
            logger.info(f"Found {total_hits} total hits, returned {len(documents)} documents")
