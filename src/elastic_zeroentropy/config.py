...
class ElasticZeroEntropyConfig(BaseSettings):
    ...
    zeroentropy_api_key: str = Field(
        ..., 
        env="ZEROENTROPY_API_KEY",
        description="ZeroEntropy API key"
    )
    ...
    @validator('zeroentropy_base_url')
    ...
    @validator('default_rerank_weight')
    def validate_combined_weights(cls, v: float, values: Dict[str, Any]) -> float:
        es_weight = values.get('default_elasticsearch_weight', 0.0)
        total = es_weight + v
    ...
    def validate_configuration(self) -> None:
        ...
        try:
            ...
            if not parsed_ze_url.scheme or not parsed_ze_url.netloc:
                errors.append(f"Invalid ZeroEntropy base URL: {self.zeroentropy_base_url}")
