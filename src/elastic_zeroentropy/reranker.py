...
class ElasticZeroEntropyReranker:
    ...
    def __init__(
        ...
        self.elasticsearch_client = elasticsearch_client or ElasticsearchClient(self.config)
        ...
        try:
            ...
            if elasticsearch_query:
                ...
                es_response = await self.elasticsearch_client.search(
                    index=index,
                    query=es_query,
                    size=reranker_config.top_k_initial
                )
            ...
            if debug_info is not None:
                debug_info["elasticsearch"] = {
                    "query": es_query if not elasticsearch_query else elasticsearch_query.query,
                    "took_ms": es_took_ms,
                    "total_hits": es_response.total_hits,
                    "returned_docs": len(es_response.documents)
                }
                
            logger.info(f"Elasticsearch returned {len(es_response.documents)} documents in {es_took_ms}ms")
            ...
            logger.info(f"Search completed: {len(final_results)} results in {total_took_ms}ms")
    ...
    def _combine_results(
        ...
        for rank, (doc, rerank_score) in enumerate(reranked_docs, 1):
            ...
            if config.combine_scores:
                ...
                normalized_es_score = self._normalize_score(es_score, es_scores.values())
