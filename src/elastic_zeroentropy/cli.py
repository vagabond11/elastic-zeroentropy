...
@click.option('--top-k-initial', type=int, default=100, help='Number of documents to retrieve from Elasticsearch')
@click.option('--top-k-rerank', type=int, default=20, help='Number of documents to send for reranking')
...
@click.option('--output', type=click.Choice(['table', 'json', 'simple']), default='table', help='Output format')
    ...
    async def run_search() -> None:
        ...
        try:
            ...
            if debug_info and e.details:
                console.print(f"[yellow]Details: {json.dumps(e.details, indent=2)}[/yellow]")
            sys.exit(1)
        except Exception as e:
...
def display_search_results(response: Any, output_format: str) -> None:
    ...
    if output_format == 'json':
        ...
        for result in response.results:
            console.print(f"[bold]{result.rank}.[/bold] [green]{result.document.title or result.document.id}[/green]")
        ...
        for result in response.results:
            text_preview = result.document.text[:100] + "..." if len(result.document.text) > 100 else result.document.text
            ...
            if response.reranking_enabled:
                es_score = f"{result.elasticsearch_score:.4f}" if result.elasticsearch_score is not None else "N/A"
                rerank_score = f"{result.rerank_score:.4f}" if result.rerank_score is not None else "N/A"
        ...
        if response.debug_info:
            console.print()
            console.print(Panel(JSON(json.dumps(response.debug_info, indent=2)), title="Debug Information"))
...
def display_health_status(health_response: Any) -> None:
    ...
    console.print(f"Overall Status: [{status_color}]{health_response.status.upper()}[/{status_color}]")
    console.print()
    ...
    if "cluster_health" in health_response.elasticsearch:
        ...
        es_panel_content += f"Cluster Status: {cluster_health.get('status', 'unknown')}\n"
        es_panel_content += f"Number of Nodes: {cluster_health.get('number_of_nodes', 'unknown')}\n"
        
    if "elasticsearch_version" in health_response.elasticsearch:
        es_panel_content += f"Version: {health_response.elasticsearch['elasticsearch_version']}\n"
...
def config_show(ctx: click.Context) -> None:
    ...
    if config_dict.get("zeroentropy_api_key"):
        config_dict["zeroentropy_api_key"] = "*" * 8 + config_dict["zeroentropy_api_key"][-4:]
    if config_dict.get("elasticsearch_password"):
        config_dict["elasticsearch_password"] = "*" * 8
    if config_dict.get("elasticsearch_api_key"):
        config_dict["elasticsearch_api_key"] = "*" * 8 + config_dict["elasticsearch_api_key"][-4:]
        ...
        try:
            ...
            async with ElasticsearchClient(config) as es_client:
                ...
                if sample_response.documents:
                    console.print()
                    console.print(f"[bold]Sample Documents ({len(sample_response.documents)}):[/bold]")
                    ...
                    console.print(f"[yellow]No documents found in index '{index}'[/yellow]")
