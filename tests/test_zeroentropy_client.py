...
class TestZeroEntropyClient:
    ...
    def test_initialization(self, mock_config):
        ...
        assert client._rate_limiter is None  # Created when rate limiting enabled
    ...
    async def test_ensure_client(self, mock_config):
        ...
        with patch('elastic_zeroentropy.zeroentropy_client.httpx.AsyncClient') as mock_client_class:
    ...
    async def test_make_request_timeout(self, mock_config):
        ...
        assert exc_info.value.details["timeout_seconds"] == mock_config.zeroentropy_timeout
    ...
    @pytest.mark.asyncio
    async def test_rerank_batch_with_semaphore(self, mock_config, mock_rerank_response_data):
        """Test batch reranking respects concurrency limits."""
        client = ZeroEntropyClient(config=mock_config)
        
        requests = [
            RerankRequest(query=f"query{i}", documents=[f"doc{i}"])
            for i in range(5)
        ]
