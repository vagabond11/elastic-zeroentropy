...
class TestElasticZeroEntropyReranker:
    ...
    async def test_context_manager(self, mock_config):
        ...
        ) as mock_es, patch.object(
            ElasticZeroEntropyReranker,
            'zeroentropy_client'
        ) as mock_ze:
            
            mock_es.__aenter__ = AsyncMock()
            mock_es.__aexit__ = AsyncMock()
            mock_ze.__aenter__ = AsyncMock()
            mock_ze.__aexit__ = AsyncMock()
            
            async with ElasticZeroEntropyReranker(config=mock_config) as reranker:
                assert reranker is not None
                
            mock_es.__aenter__.assert_called_once()
            mock_es.__aexit__.assert_called_once()
            mock_ze.__aenter__.assert_called_once()
            mock_ze.__aexit__.assert_called_once()
    ...
    async def test_rerank_documents_success(
        ...
        config = RerankerConfig()
        result = await reranker._rerank_documents("test query", sample_documents, config)
    ...
    def test_normalize_score(self, mock_config):
        ...
        assert reranker._normalize_score(0.55, scores) == 0.5  # middle
